        ...
        if let responseString = String(data: data, encoding: .utf8) {
            ...
            if json!["Success"] as? Int ?? 0 ==  0{
                ...
                DispatchQueue.main.async {
                    ...
                    self.lastUpdateTİme.text = "Last Updated Time \(self.customerStores.LastUpdate[0].dropLast(6))"
                    self.setupPieChart()
...
func setupPieChart() {
    ...
    for i in 0..<customerStores.Format.count {
        entriesStores.append(PieChartDataEntry(value: self.customerStores.Ciro[i] , label: ""))
    }
    for i in 0..<customerCategory.Category.count {
        entriesChannel.append(PieChartDataEntry(value: self.customerCategory.Ciro[i] , label: ""))
    }
